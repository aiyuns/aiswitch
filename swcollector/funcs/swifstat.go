...
func initVariable() {
	...
	ignoreVlanStatTotal = g.Config().Switch.IgnoreVlanStatTotal
}
			...
			for _, ifStat := range *chIfStat.IfStatsList {
				...
				if !ignoreVlanStatTotal {
					if displayByBit {
						...
						L = append(L, CounterValueIp(ifStat.TS, ip, "switch.hw.VlanIn", ifStat.HWL2VlanStatInTotalBytes, ifNameTag, ifIndexTag))
						L = append(L, CounterValueIp(ifStat.TS, ip, "switch.hw.VlanOut", ifStat.HWL2VlanStatOutTotalBytes, ifNameTag, ifIndexTag))
					}

					if !ignorePkt {
						L = append(L, CounterValueIp(ifStat.TS, ip, "switch.hw.VlanInPkts", ifStat.HWL2VlanStatInTotalPkts, ifNameTag, ifIndexTag))
						L = append(L, CounterValueIp(ifStat.TS, ip, "switch.hw.VlanOutPkts", ifStat.HWL2VlanStatOutTotalPkts, ifNameTag, ifIndexTag))
					}
				}
			}
		}
...
func coreSwIfMetrics(ip string, ch chan ChIfStat, limitCh chan bool) {
	...
	if !pingResult {
		...
		if vendor == "Huawei_V8.80" {
			ifList, err = sw.ListIfStatsHuaweiHasVlanOid(ip, community, snmpTimeout, ignoreIface, snmpRetry, ignorePkt, ignoreOperStatus, ignoreBroadcastPkt, ignoreMulticastPkt, ignoreVlanStatTotal)
		} else {
